...
- `Character.refs.afflictions.GetCurrentStatus(STATUSTYPE.Injury)`
...
1. Скопируйте DLL из игры/модов в папку `lib` рядом с `.csproj`.  
   - Если в папке игры есть только `HarmonyXInterop.dll` (часто так в новых сборках BepInEx), просто скопируйте её — проект подтянет её автоматически.  
   - Если есть `HarmonyX.dll`, можно использовать её вместо `HarmonyXInterop.dll`.  
   - `UnityEngine.InputLegacyModule.dll` больше не обязателен для сборки (ввод читается через reflection), но если он есть — можно тоже скопировать.
